...
def material_evaluation(fish: Stockfish) -> int:
    ...
    for piece in pieces:
        if piece_values.get(piece):
            evaluation += piece_values[piece]*pieces[piece]
    return evaluation

def get_evaluation_array(san_moves: List[str]) -> List[dict]:
...
def get_blunders(san_moves: List[str]) -> List[list]:
    ...
    for i in range(len(eval_array) - 1):
        ...
        # Change in game state of 2 or greater indicates a blunder
            ...
            blunders.append([san_moves[i + 1], uci_moves[i + 1], (i + 1) % 2, (i // 2) + 1, state_array[i + 1], state_array[i]] + list(eval_array[i + 1].values()) + list(eval_array[i].values()))
...
def get_monthly_blunders(date: datetime) -> None:
